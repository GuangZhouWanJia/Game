...
// main stage 初始化方法
	...
	TD.getDefaultStageData = function (k) {
		var data = {
			stage_main: {
				width: ((TD.Col)*50) * _TD.retina, // px 640		(-1)*(TD.Col)		
			...
			scene_endless: {
				// scene 1
				map: {
					grid_x: TD.Col, //16 (-1)*(TD.Col)
					grid_y: 16,
					x: TD.padding,
					y: TD.padding,
					entrance: [0, 0],
					exit: [TD.Col-1, 15], //15+(-1)*(TD.Col)
					grids_cfg: [
						{
							pos: [3, 3],
							//building: "cannon",
				...
				panel: {
					x: TD.padding * 2 + TD.grid_size * TD.Col, //游戏属性列定义(16+(-1)*(TD.Col))
					y: TD.padding,
					map: {
						grid_x: 3,
						grid_y: 3,
